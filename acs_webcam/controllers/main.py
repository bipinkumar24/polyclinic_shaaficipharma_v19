...
class AcsWebcam(http.Controller):
    ...
    def acs_webcam(self, model, record_id, image_field='', **kw):
        ...
        values = { 'record': record.id, 
            ...
            'action': record.acs_webcam_return_action(),
        }
    ...
    def acs_webcam_update_image(self, model, record_id, **kwargs):
        ...
        redirect_url = '/web#model=%s&id=%s&action=%s&view_type=form' % (record._name, record.id, record.acs_webcam_return_action())
