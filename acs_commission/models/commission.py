...
class PartnerCommission(models.Model):
    ...
    commission_on = fields.Float('Base Amount')
    ...
    acs_invoice_line_ids = fields.Many2many('account.move.line', 'acs_invoice_line_commission_rel', 'invoice_line_id', 'commission_id', 'Invoice Lines', copy=False)
    ...
    commission_sheet_id = fields.Many2one('acs.commission.sheet', 'Sheet')
    ...
    def create(self, vals_list):
        for vals in vals_list:
            if vals.get('name', _("New")) == _("New"):
                ...
                vals['name'] = self.env['ir.sequence'].with_company(vals.get('company_id')).next_by_code('acs.commission', sequence_date=seq_date) or _("New")
