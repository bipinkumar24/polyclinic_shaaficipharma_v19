...
class AcsCommissionSheet(models.Model):
    ...
    partner_id = fields.Many2one('res.partner', string='Partner', index=True, required=True, tracking=True, domain="[('provide_commission','=',True)]")
        ...
        for vals in vals_list:
            if vals.get('name', _("New")) == _("New"):
                ...
                if vals.get('date_from'):
                    seq_date = fields.Datetime.context_timestamp(self, fields.Datetime.to_datetime(vals['date_from']))
                vals['name'] = self.env['ir.sequence'].with_company(vals.get('company_id')).next_by_code('acs.commission.sheet', sequence_date=seq_date) or _("New")
