...
class ACSCommissionBill(models.TransientModel):
    ...
    def create_bills(self):
        ...
        if self.commission_sheet_ids:
            ...
            for commission_sheet_id in self.commission_sheet_ids:
                if commission_sheet_id.payment_invoice_id:
                    raise UserError(_('Already Bill is created for ' + commission_sheet_id.name))
        else:
        ...
        if any(line.target_based_commission and not line.commission_sheet_id for line in lines):
            raise UserError(_('Commission Bill Can be created form commission sheet only for target based commissions.'))
        ...
        if self.groupby_partner:
            groupby = 'partner_id'
        if groupby:
            commission_group = Commission.read_group([('id', 'in', lines.ids),
        ...
        if not bills:
            raise UserError(_('Please check there is nothing to bill in selected Commission may be you are missing partner or trying to bill already billed Commissions.'))

        #last bill from above loop will be used
        if self.env.context.get('commission_sheet_id', False):
            sheet = self.env['acs.commission.sheet'].search([('id','=',self.env.context.get('commission_sheet_id', False))])
            sheet.payment_invoice_id = bill.id
        if self.commission_sheet_ids:
            for commission_sheet_id in self.commission_sheet_ids:
                commission_sheet_id.payment_invoice_id = bill.id
