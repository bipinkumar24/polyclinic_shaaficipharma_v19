...
class ReportSaleDetailsinherit(models.AbstractModel):
	...
	def get_sale_details(self, date_start=False, date_stop=False, config_ids=False, session_ids=False):
		...
		for category_name, product_list in products_sold.items():
			category_dictionnary = {
				...
				} for (product, price_unit, discount,second_uom), (qty, product_total, base_amount,second_uom) in product_list.items()], key=lambda l: l['product_name']),
			}
	...
	def _get_products_and_taxes_dict(self, line, products, taxes, currency):
		key2 = (line.product_id, line.price_unit, line.discount,line.custom_uom_id)
		...
		if key1 not in products:
			products[key1] = {}

		if key2 not in products[key1]:
			products[key1][key2] = [0.0, 0.0, 0.0, '']
		...
		if line.custom_uom_id:
			products[key1][key2][3] = line.custom_uom_id
		else:
			products[key1][key2][3] = line.uom_id.name
