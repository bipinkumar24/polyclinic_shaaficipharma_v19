...
NATURAL_KEYS = {
    ...
    # x_v18_name holds the v18 Test ID verbatim and is the most reliable
    # cross-version key (unaffected by local sequences), so it is matched first.
    "patient.laboratory.test": ["x_v18_name", "unique_code", "name"],
    "hms.patient": ["code", "gov_code"],
    # Radiology: x_v18_name (the v18 Request Number / Test ID) is the reference
    # key, matched before anything else so re-syncs never duplicate.
    "acs.radiology.request": ["x_v18_name", "name"],
    "patient.radiology.test": ["x_v18_name", "unique_code", "name"],
}

# Store the remote record's `name` into this local field (identity reference
# across versions). Populated for every synced record and used for dedup.
V18_NAME_FIELD = {
    "patient.laboratory.test": "x_v18_name",
    "acs.radiology.request": "x_v18_name",
    "patient.radiology.test": "x_v18_name",
}

# Models whose local `name` is generated by the system (a sequence) and must
# NOT be copied from the remote. Their remote name is stored only in
# V18_NAME_FIELD; here we suppress the "Unnamed" fallback so the local create
# default / sequence assigns the name. (Contrast with FORCE_COPY_FIELDS, which
# copies the remote name verbatim, e.g. for lab tests.)
NAME_AUTOGENERATED_MODELS = {
    "acs.radiology.request",
    "patient.radiology.test",
}

# Models synced with the `avoid_subsequent_test` context so that creating a
# request line does not auto-append the test's subsequent-test lines (they are
# already present in the remote data being copied -> would duplicate).
AVOID_SUBSEQUENT_TEST_MODELS = {
    "acs.radiology.request",
}

# Comodels that must be FULLY synced (all fields, not a name-only stub) when
# referenced by a many2one and not found locally. e.g. a lab test's patient_id:
# create the complete patient record from the remote, then link it.
FULL_SYNC_COMODELS = {
    "hms.patient",
}

# one2many fields to synchronize as child records (create + link via the
# field's inverse). Only the fields listed here are handled; other o2m
# (activity_ids, message_ids, self-referential child_test_ids, ...) are skipped.
# Each sync REPLACES the child set (clear + recreate) so it always mirrors the
# source. Self-referential parent/child links are carried by the parent m2o.
O2M_SYNC_FIELDS = {
    "patient.laboratory.test": {"critearea_ids", "consumable_line_ids"},
    # Radiology request children: the test lines and the per-patient results.
    "acs.radiology.request": {"line_ids", "patient_test_ids"},
}

# Models for which a missing many2one target is auto-created (stub by name)
# instead of left empty. Needed where the m2o is required (e.g. lab test
# patient_id/test_id). Still guarded by NO_CREATE_COMODELS.
M2O_CREATE_MISSING = {
    "patient.laboratory.test",
    "lab.test.critearea",
    "hms.consumable.line",
    # Radiology: stub-create required targets (e.g. a line's test_id) so a
    # request/line/result is never dropped for a missing required relation.
    "acs.radiology.request",
    "radiology.request.line",
    "patient.radiology.test",
}
...
NO_CREATE_COMODELS = {
    ...
    "res.company",
    "calendar.event",
    # requires a patient_id (not-null) that a name-only stub can't supply
    "acs.patient.laboratory.sample",
    # requires a test_id (not-null): the radiology results are created via the
    # request's patient_test_ids one2many (which supplies test_id); a bare
    # name-only stub from the line's patient_radiology_ids m2m would violate it.
    "patient.radiology.test",
}
...
EXTRA_READ_FIELDS = {
    ...
    # the lab test "Test ID" (name) is readonly (excluded from the plan) but we
    # must copy the remote value so records keep their identity and don't all
    # collapse onto the "New/Unnamed" default (unique per company).
    "patient.laboratory.test": ["name"],
    # radiology name (Request Number / Test ID) is readonly: read it only to
    # store in x_v18_name and to dedup — the local name stays system-generated.
    "acs.radiology.request": ["name"],
    "patient.radiology.test": ["name"],
}

# Readonly scalar fields to copy verbatim from the remote anyway (they carry
# real identity/data despite being readonly locally, e.g. a sequence-assigned
# name). Read via EXTRA_READ_FIELDS, written here.
FORCE_COPY_FIELDS = {
    "patient.laboratory.test": ["name"],
}
...
class ProductRemoteServer(models.Model):
    ...
    def _resolve_m2o(self, field_name, comodel, value, cache, allow_create=False):
        ...
        one). Otherwise fall back to matching by name. When allow_create is set
        and no match is found, a stub record is created by name (so required
        relations like a lab test's patient/test never leave the record failing).
        """
        if not value:
            return False
        ...
        if remote_id and "remote_id" in Model._fields:
            ...
            if rec:
                cache[key] = rec.id
                return rec.id
        # Full-sync comodels (e.g. hms.patient): if not already mapped, create
        # the COMPLETE record from the remote (all fields) and link it, rather
        # than a name-only stub. Deduped by remote-id / natural key inside.
        if comodel in FULL_SYNC_COMODELS and remote_id and "__conn__" in cache:
            key = (comodel, "rid", remote_id)
            if key in cache and cache[key]:
                return cache[key]
            conn_uid, conn_proxy = cache["__conn__"]
            rec = self._sync_related_record(
                comodel, remote_id, conn_uid, conn_proxy, cache
            )
            if rec:
                cache[key] = rec.id
                return rec.id
        if not name:
            return False
        key = (comodel, name)
        if key in cache and not allow_create:
            return cache[key]
        if key in cache and cache[key]:
            return cache[key]
        ...
        if field_name == "categ_id":
            ...
            if (
                ...
                result = self._stub_create(comodel, name)
        cache[key] = result
        return result

    def _stub_create(self, comodel, name, depth=0):
        """Create a minimal ("stub") record for a missing related target.

        Fills required fields so the create succeeds even when the target model
        has its own required relations (e.g. acs.lab.test requires product_id):
        required many2one are satisfied by reusing an existing record or
        recursively stubbing (depth-limited); required scalars get a safe
        placeholder. Runs in a savepoint so a failure never poisons the outer
        transaction. Returns the new id, or False if it still can't be created.
        """
        Model = self.env[comodel].sudo()
        create_vals = {}
        if "name" in Model._fields:
            create_vals["name"] = name or "SYNC"
        try:
            defaults = Model.default_get(list(Model._fields))
        except Exception:
            defaults = {}
        for fname, field in Model._fields.items():
            if fname in create_vals or fname in EXCLUDED_FIELDS:
                continue
            if not field.store or field.readonly or field.compute:
                continue
            if not getattr(field, "required", False):
                continue
            if defaults.get(fname) not in (None, False):
                continue  # a default will populate it
            ftype = field.type
            if ftype == "many2one":
                co = field.comodel_name
                if co in NO_CREATE_COMODELS or depth >= 3:
                    existing = self.env[co].sudo().search([], limit=1)
                    if existing:
                        create_vals[fname] = existing.id
                    continue
                existing = self.env[co].sudo().search([], limit=1)
                create_vals[fname] = (
                    existing.id if existing
                    else self._stub_create(co, name, depth + 1)
                )
            elif ftype in ("char", "text", "html"):
                create_vals[fname] = name or "SYNC"
            elif ftype == "selection":
                sel = field.selection
                sel = sel(Model) if callable(sel) else sel
                if sel:
                    create_vals[fname] = sel[0][0]
        ...
        try:
            with self.env.cr.savepoint():
                return Model.create(create_vals).id
        except Exception as exc:
            _logger.info(
                "Remote sync: could not stub-create %s '%s': %s",
                comodel, name, exc,
            )
            return False
    ...
    def _resolve_or_create(self, comodel, name, cache):
        ...
        try:
            ...
            if (
                ...
                # Savepoint so a failed create doesn't poison the outer tx.
                with self.env.cr.savepoint():
    ...
    def _presync_m2o(self, model, plan, records, cache):
        """Resolve/create every m2o target for a batch up front (main tx).

        Auto-created targets (required stubs, full-sync records) are made here,
        outside any per-record savepoint, so they persist even if an individual
        record fails and rolls back — the cache then always points at a live id.
        """
        Model = self.env[model]
        allow = model in M2O_CREATE_MISSING
        for fname, comodel in plan["m2o"].items():
            required = getattr(Model._fields.get(fname), "required", False)
            for data in records:
                if not data.get(fname):
                    continue
                # own savepoint per target: released on success (target persists),
                # rolled back only if that target itself fails — never poisons the
                # batch or leaves a stale cached id.
                try:
                    with self.env.cr.savepoint():
                        self._resolve_m2o(
                            fname, comodel, data.get(fname), cache,
                            allow_create=allow and required,
                        )
                except Exception as exc:
                    _logger.warning(
                        "Remote sync: pre-sync of %s.%s failed: %s",
                        model, fname, exc,
                    )

    def _build_o2m_commands(self, uid, proxy, model, plan, records, cache):
        """Build one2many write commands per parent for a batch of records.

        Returns {(field_name, parent_remote_id): [(5, 0, 0), (0, 0, child_vals), ...]}.
        Child records are read once per comodel from the remote and mapped
        through the same engine (scalars + m2o + m2m); the inverse link field is
        dropped (Odoo sets it via the o2m). (5, 0, 0) clears existing children so
        each sync REPLACES the set — always mirroring the source.
        """
        commands = {}
        Parent = self.env[model]
        for field_name, comodel in plan["o2m"].items():
            inverse = Parent._fields[field_name].inverse_name
            child_ids = set()
            for data in records:
                child_ids.update(data.get(field_name) or [])
            # default: clear children for every parent (replace semantics)
            for data in records:
                commands[(field_name, data["id"])] = [(5, 0, 0)]
            if not child_ids:
                continue
            child_plan = self._get_field_plan(uid, proxy, comodel)
            child_plan["o2m"] = {}  # never recurse into grandchildren
            child_plan["m2o"].pop(inverse, None)  # inverse set via the o2m link
            rows = self._execute(
                uid, proxy, comodel, "read", [list(child_ids)],
                {"fields": self._read_fields(child_plan)},
            )
            child_data = {row["id"]: row for row in rows}
            child_m2m = self._fetch_m2m_names(uid, proxy, child_plan, rows)
            for data in records:
                cmds = [(5, 0, 0)]
                for cid in data.get(field_name) or []:
                    cdata = child_data.get(cid)
                    if not cdata:
                        continue
                    cvals = self._prepare_vals(
                        comodel, cdata, child_plan, cache, child_m2m
                    )
                    cvals.pop(inverse, None)
                    cvals.pop("remote_server_id", None)
                    cvals.pop("remote_id", None)
                    cmds.append((0, 0, cvals))
                commands[(field_name, data["id"])] = cmds
        return commands
    ...
    def _get_field_plan(self, uid, proxy, model):
        ...
        plan = {
            "scalar": [], "m2o": {}, "m2m": {}, "o2m": {},
            "remote_fields": set(remote_fields),
        }
        model_excluded = MODEL_EXCLUDED_FIELDS.get(model, ())
        allowed_o2m = O2M_SYNC_FIELDS.get(model, set())
        skipped_o2m = []
        for name, meta in local_fields.items():
            ...
            if not field:
                continue
            # Skip _inherits-delegated fields (e.g. hms.patient's name/phone/
            # address live on res.partner): the parent record carries them and
            # is synced via the delegate m2o, so writing them here would clobber
            # the linked parent.
            if getattr(field, "inherited", False):
                continue
            ftype = meta.get("type")
            # one2many: only the explicitly configured ones are synced as child
            # records (they may be non-stored computed fields with an inverse).
            if ftype == "one2many":
                if name in allowed_o2m and field.inverse_name and not field.readonly:
                    plan["o2m"][name] = field.comodel_name
                else:
                    skipped_o2m.append(name)
                continue
            # everything else must be a writable, stored, real column: skips
            # computed/related and mail/activity/quantity technical fields.
    ...
    def _prepare_vals(self, model, data, plan, cache, m2m_names, o2m_commands=None):
        ...
        Model = self.env[model]
        allow_create = model in M2O_CREATE_MISSING
        vals = {}
        ...
        for name, comodel in plan["m2o"].items():
            # Only stub-create a missing target for REQUIRED m2o (e.g. lab test
            # test_id/patient_id). Optional references (appointment_id,
            # request_id, ...) are linked only if the record already exists;
            # otherwise they are left empty rather than fabricated.
            field_required = getattr(Model._fields.get(name), "required", False)
            rid = self._resolve_m2o(
                name, comodel, data.get(name), cache,
                allow_create=allow_create and field_required,
            )
            if rid:  # omit unresolved m2o so local defaults / required apply
                vals[name] = rid
        for name in plan.get("o2m", {}):
            cmds = (o2m_commands or {}).get((name, data["id"]))
            if cmds is not None:
                vals[name] = cmds
        ...
        # force-copy readonly identity fields (e.g. lab test name/Test ID)
        for fname in FORCE_COPY_FIELDS.get(model, ()):
            if fname in data and fname in Model._fields:
                vals[fname] = data.get(fname) or False
        # store the remote record's name into the cross-version reference field
        v18_field = V18_NAME_FIELD.get(model)
        if v18_field and v18_field in Model._fields:
            vals[v18_field] = data.get("name") or False
        name_field = Model._fields.get("name")
        if (
            name_field
            and not getattr(name_field, "inherited", False)
            and not vals.get("name")
            and model not in NAME_AUTOGENERATED_MODELS
        ):
            vals["name"] = _("Unnamed")
    ...
    def _safe_create(self, model, vals):
        ...
        try:
            ...
            # Uniqueness fallback: a record with the same name already exists
            # (e.g. a lab test with the same Test ID from a prior/partial sync).
            # Treat it as the same record and update it instead of failing.
            Model = self.env[model].with_context(active_test=False)
            if vals.get("name") and "name" in Model._fields:
                existing = Model.search([("name", "=", vals["name"])], limit=1)
                if existing:
                    _logger.info(
                        "Remote sync (%s): name %r exists, updating existing #%s",
                        model, vals["name"], existing.id,
                    )
    ...
    def _find_existing(self, model, data):
        ...
        if record:
            return record
        # cross-version reference: match the stored v18 name (x_v18_name) against
        # the remote record's name — the most reliable identity key.
        v18_field = V18_NAME_FIELD.get(model)
        if v18_field and data.get("name") and v18_field in Model._fields:
            record = Model.search([(v18_field, "=", data["name"])], limit=1)
            if record:
                return record
        ...
        for key in NATURAL_KEYS.get(model, []):
            if key == v18_field:
                continue  # handled above (mapped to remote 'name')
            value = data.get(key)
            if value:
    ...
    # -- Lab Test Results ------------------------------------------------
    def action_sync_lab_tests(self):
        return self._run_sync("patient.laboratory.test")

    def action_sync_one_lab_test(self):
        """Test helper: synchronize a single lab test from the remote server."""
        return self._run_sync("patient.laboratory.test", limit=1)

    def action_sync_lab_test_batch(self):
        """Test helper: synchronize the first `test_limit` lab tests."""
        self.ensure_one()
        return self._run_sync("patient.laboratory.test", limit=self.test_limit or 1)

    # -- Radiology Requests ---------------------------------------------
    def action_sync_radiology(self):
        return self._run_sync("acs.radiology.request")

    def action_sync_one_radiology(self):
        """Test helper: synchronize a single radiology request."""
        return self._run_sync("acs.radiology.request", limit=1)

    def action_sync_radiology_batch(self):
        """Test helper: synchronize the first `test_limit` radiology requests."""
        self.ensure_one()
        return self._run_sync("acs.radiology.request", limit=self.test_limit or 1)
    ...
    def _sync_related_record(self, model, remote_id, uid, proxy, cache, seen=None):
        ...
        # make the connection available to nested m2o resolution
        cache.setdefault("__conn__", (uid, proxy))
        ...
        if not rows:
            return Empty
        data = rows[0]
        # dedup against an already-existing record by natural key (avoid
        # duplicates); if found, stamp the remote-id mapping and reuse it.
        for nkey in NATURAL_KEYS.get(model, []):
            nval = data.get(nkey)
            if nval and nkey in Model._fields:
                match = Model.search([(nkey, "=", nval)], limit=1)
                if match:
                    match.write({
                        "remote_server_id": self.id, "remote_id": remote_id,
                    })
        ...
        # hms.patient _inherits res.partner: fully sync the partner first so all
        # delegated data (name, phone, address, ...) is carried, then link it.
        if model == "hms.patient" and data.get("partner_id"):
            self._sync_related_record(
                "res.partner", data["partner_id"][0], uid, proxy, cache, seen
            )
        m2m_names = self._fetch_m2m_names(uid, proxy, plan, [data])
        vals = self._prepare_vals(model, data, plan, cache, m2m_names)
        return self._safe_create(model, vals)
    ...
    def _sync_model(self, model, batch_size=None, limit=None):
        ...
        """
        self.ensure_one()
        if model in AVOID_SUBSEQUENT_TEST_MODELS:
            # Creating a radiology request line auto-appends the test's
            # subsequent-test lines; those are already in the remote data being
            # copied, so suppress the automatic ones to avoid duplicates.
            self = self.with_context(avoid_subsequent_test=True)
        batch_size = batch_size or self.batch_size or 100
        ...
        cache["__conn__"] = (uid, proxy)  # for full-sync m2o (e.g. patient_id)
        # Only filter on active when the remote model actually has that field
        # (some models, e.g. patient.laboratory.test, have no active field).
        if self.sync_inactive or "active" not in plan["remote_fields"]:
            domain = []
        else:
            domain = [("active", "=", True)]
        offset = 0
        ...
        while True:
            ...
            # Pre-create referenced m2o targets (auto-created stubs + full-sync
            # records like patients) in the main transaction, BEFORE the
            # per-record loop. This way a later record's savepoint rollback can't
            # undo a shared target while the cache still points at it (which
            # would cause a foreign-key violation on the next record).
            self._presync_m2o(model, plan, records, cache)
            o2m_commands = self._build_o2m_commands(
                uid, proxy, model, plan, records, cache
            ) if plan.get("o2m") else {}
            for data in records:
                try:
                    with self.env.cr.savepoint():
                        vals = self._prepare_vals(
                            model, data, plan, cache, m2m_names, o2m_commands
                        )
