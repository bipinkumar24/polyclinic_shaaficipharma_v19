...
class AccountPartnerLedger(models.AbstractModel):
    ...
    #     # Collect all move line ids
    #     all_aml_ids = []
    #     for amls in aml_values.values():
    #         for aml in amls:
    #             if aml.get('id'):
    #                 all_aml_ids.append(aml['id'])

    #     if not all_aml_ids:
    #         return aml_values

    #     # Fetch branch_id in batch (PERFORMANCE SAFE)
    #     move_lines = self.env['account.move.line'].browse(all_aml_ids)
    #     branch_mapping = {ml.id: ml.branch_id.name if ml.branch_id else '' for ml in move_lines}

    #     # Inject branch name into report values
    #     for partner_id, amls in aml_values.items():
    #         for aml in amls:
    #             aml['branch_name'] = branch_mapping.get(aml.get('id'), '')
