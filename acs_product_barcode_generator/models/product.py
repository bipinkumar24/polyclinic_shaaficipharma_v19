...
class ir_sequence(models.Model):
    ...
    acs_ean13 = fields.Boolean('EAN13 Barcode', default=False)
...
class ProductTemplate(models.Model):
    ...
    def create(self, vals_list):
        ...
        for rec in res:
            if not rec.barcode and self.env.company.auto_create_barcode:
...
class ProductProduct(models.Model):
    ...
    def _get_barcode_next_code(self):
        ...
        if product.barcode_sequence_id:
            barcode_sequence = product.barcode_sequence_id
            ...
            barcode_sequence = product.categ_id.barcode_sequence_id
            ...
            barcode_sequence = product.company_id.barcode_sequence_id
            barcode = product.company_id.barcode_sequence_id.next_by_id()
            prefix = product.company_id.barcode_sequence_id.prefix
        elif self.env.company.barcode_sequence_id:
            barcode_sequence = self.env.company.barcode_sequence_id
            barcode = self.env.company.barcode_sequence_id.next_by_id()
            prefix = self.env.company.barcode_sequence_id.prefix
        else:
            raise UserError(_('Configure Barcode seq on Product or Product Category or on Company.'))

        if barcode_sequence.acs_ean13:
    ...
    def _generate_barcode_value(self):
        ...
        matching_products = self.env['product.product'].search([('barcode', '=', barcode)])
