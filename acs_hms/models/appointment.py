...
class Appointment(models.Model):
    ...
    #     """
    #     Ensure today's date-range exists for the daily sequence.
    #     This calls the private helper `_create_date_range` if available.
    #     """
    #     seq = self.env['ir.sequence'].sudo().search([('code', '=', 'daily.custom.sequence')], limit=1)
    #     if not seq:
    #         return True
    ...
    daily_number = fields.Char(string='Daily Number', readonly=True, copy=False, tracking=1, default='New')
    ...
    is_wake_up = fields.Boolean(string="Walk-in")
    ...
    is_charge_the_appoinmnet_fee = fields.Boolean(string="Charge The Appointment Fee")
    ...
    @api.depends('patient_id', 'patient_id.treatment_ids.state')
    def _compute_treatment_count(self):
        for rec in self:
            if rec.patient_id:
                treatments = rec.patient_id.treatment_ids.filtered(
                    lambda trt: trt.state in ['draft', 'running']
                )
                rec.treatment_count = len(treatments)
            else:
                rec.treatment_count = 0

    @api.onchange('is_wake_up')
    def onchange_is_wake_up(self):
        for rec in self:
            if rec.is_wake_up:
    ...
    def create(self, vals_list):
        ...
        #     if vals.get('daily_number', _("New")) == _("New"):
        #         seq = self.env['ir.sequence'].next_by_code('daily.custom.sequence')
        #         vals['daily_number'] = seq or 'New'
    ...
    def acs_appointment_inv_product_data(self, with_product=True):
        ...
        if with_product and not self.invoice_exempt:
            ...
            if self.is_charge_the_appoinmnet_fee:
    ...
    def create_invoice(self):
        ...
        # invoice = self.with_context(acs_context).acs_create_invoice(
        ...
        data = self.acs_appointment_common_wizard_data(False)
        if not self.is_invoice and self.is_charge_the_appoinmnet_fee:
            data += self.acs_appointment_inv_product_data()
        line_vals = []
        for line in data:
            # skip section lines that only contain name
            ...
            line_vals.append((0, 0, {
                ...
                # 'laboratory_request_line_id': line.get('laboratory_request_line_id'),
                # 'radiology_request_line_id': line.get('radiology_request_line_id'),
            }))
        print(line_vals, "line_valssssssssssss")
        return {
            'name': 'Create Invoice',
            'type': 'ir.actions.act_window',
            'res_model': 'appointment.invoice.wizard',
            'view_mode': 'form',
            'target': 'new',
            'context': {
